# World-building implementation plan (MotionGram)

This document describes **how** to grow MotionGram toward compositional “world” authoring—few primitives in combination, timeline-driven motion—and **what already exists** in the repo so new work stays aligned with [AGENTS.md](AGENTS.md).
...
- **Spatial:** Build scenes from a small vocabulary of drawables grouped under [`Node`](src/motiongram/core.py): parent-relative coordinates, painter order, no hidden globals. Use [`motiongram.world`](src/motiongram/world.py) when coordinates should be **logical** (stage space) rather than pixels.
- **Temporal:** Schedule clips with [`Scene.add_animation`](src/motiongram/core.py) and compose behavior with leaf [`Animator`](src/motiongram/animate.py)s plus `Parallel`, `Sequence`, `Delay`, and principle-style wrappers—**not** ad hoc motion inside `Node.update` for layout.
...
|-------|----------------|---------------|
| Structure | Scene graph, poses, `draw` | `Node`, shapes under [`motiongram.shapes`](src/motiongram/shapes.py), [`motiongram.form`](src/motiongram/form.py), [`motiongram.world`](src/motiongram/world.py) |
...
1. **`WorldPortal`** maps a subtree whose `x` / `y` are world units into **frame pixel** coordinates using `SkiaCanvas.push_affine_2x3` (see [`render.py`](src/motiongram/render.py)).
2. **`SkiaRenderer.render_frame`** applies [`Scene.camera`](src/motiongram/core.py) (pan/zoom/rotate around the frame center) in **pixel** space, unchanged from before.
...
Implemented under [`src/motiongram/shapes.py`](src/motiongram/shapes.py) and drawn via [`SkiaCanvas`](src/motiongram/render.py) helpers:

- **Strokes / paths:** `Line`, `BezierCurve`, `Arc`, `Path`
- **Fills:** `Polygon`, `Rectangle`, `Ellipse`, `RegularPolygon`
- **Circular wedges (filled):** `Sector`, `SemiCircle` — use [`fill_sector`](src/motiongram/render.py) on Skia; full-turn wedges delegate to ellipse fill.
...
- **World containers** — [`world_shell()`](src/motiongram/world.py) returns named nodes for background / mid / foreground and character/prop grouping; sibling order defines painter order (no automatic global sort on `world_z` in v1).
- **Approximate rings** — segment polylines with `Line` ([`examples/showcase_intro.py`](examples/showcase_intro.py) `_ring`).
- **Higher-level layout helpers** — [`motiongram.composition`](src/motiongram/composition.py) (`align`, `distribute_evenly`, `stack_vertical`, grids/spirals where applicable).
...
[`src/motiongram/recipes.py`](src/motiongram/recipes.py) documents small helpers that **only** call `scene.add_animation`:
...
### Procedural manifests (optional `motiongram.procedural`)

- **Purpose:** seedable, data-first descriptions (manifests) of parametrized worlds—terrain silhouettes, weather fields, discrete props—that **materialize** into [`world_shell()`](src/motiongram/world.py) buckets and return handles for wiring [`Scene.add_animation`](src/motiongram/core.py).
- **Not core engine:** authors import explicitly (`from motiongram.procedural import …`). Symbols are **not** re-exported from the `motiongram` package root so the stable public surface stays small.
- **Reference:** [`RainyLandscapeManifest`](src/motiongram/procedural/rainy_landscape.py) and [`examples/principles/26_world_viewport.py`](examples/principles/26_world_viewport.py).

### Compiler boundary (e.g. Manimator)

MotionGram is the **substrate**: `WorldSpec`, `WorldPortal`, the `Node` graph, discrete timeline entries (`start`, `end`, `target`, `Animator`). A higher-level toolchain (working name **Manimator**) should own narrative or curriculum intermediate representation—mapping pedagogical intent to manifests or directly to lowered scenes—and emit **fragments that compile to**:

- instantiated geometry under an existing portal/shell, and
- concrete `Animator` constructions passed to `add_animation`,

without growing topic logic inside [`motiongram.world`](src/motiongram/world.py), [`SkiaRenderer`](src/motiongram/render.py), or unrelated core modules.
...
- **Video:** [`PyAVEncoder`](src/motiongram/export.py), [`SkiaRenderer`](src/motiongram/render.py).
...
- [x] **Procedural manifest slice** — [`motiongram.procedural`](src/motiongram/procedural/__init__.py) (`RainyLandscapeManifest`, materialize + timeline helpers; see [26_world_viewport.py](examples/principles/26_world_viewport.py)).
...
2. **Keep timeline helpers thin** — only `scene.add_animation(...)` (pattern in [`recipes.py`](src/motiongram/recipes.py)); avoid hiding targets or global clocks.
...
| [src/motiongram/world.py](src/motiongram/world.py) | World units, portal, authoring stubs |
| [src/motiongram/recipes.py](src/motiongram/recipes.py) | Timeline snippet helpers |
