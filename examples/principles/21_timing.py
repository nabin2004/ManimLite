...
from motiongram import MoveX
from motiongram.shapes import Ellipse


def build_scene() -> Scene:
    scene = Scene(width=WIDTH, height=HEIGHT, fps=FPS, duration=DURATION)
    fast = Ellipse(x=140, y=240, rx=46, ry=46, fill_color="#A51C30", stroke_color="#FFFFFF", stroke_width=2.0)
    slow = Ellipse(x=140, y=380, rx=46, ry=46, fill_color="#5DD2E8", stroke_color="#FFFFFF", stroke_width=2.0)
    scene.add_node(fast)
    scene.add_node(slow)
