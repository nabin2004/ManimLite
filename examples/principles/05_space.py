...
from typmotion import FadeIn, MoveX
from typmotion.shapes import Ellipse


def build_scene() -> Scene:
    scene = Scene(width=WIDTH, height=HEIGHT, fps=FPS, duration=DURATION)
    far = Ellipse(x=480, y=280, rx=180, ry=110, fill_color="#354460", stroke_color=None, stroke_width=0.0)
    far.opacity = 0.35
    far.blur_sigma = 6.0
    mid = Ellipse(x=520, y=300, rx=120, ry=75, fill_color="#5F7AAF", stroke_color="#8FA3C4", stroke_width=1.0)
    mid.opacity = 0.75
    near = Ellipse(x=580, y=330, rx=70, ry=48, fill_color="#F0C060", stroke_color="#FFFFFF", stroke_width=2.0)
    near.opacity = 1.0
    scene.add_node(far)
    scene.add_node(mid)
    scene.add_node(near)
