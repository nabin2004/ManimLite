...
def build_scene() -> Scene:
    ...
    pie = Sector(x=240.0, y=280.0, radius=90.0, start_angle=-math.pi / 4, sweep_angle=math.pi * 1.25, fill_color="#61AFEF", stroke_color="#24365A", stroke_width=2.5)
    scene.add_node(pie)

    face = Node(x=620.0, y=260.0)
    face_base = Ellipse(x=0.0, y=0.0, rx=100.0, ry=92.0, fill_color="#F5D4A8", stroke_color="#9B7356", stroke_width=2.0)
    face.add(face_base)
    ...
    face.add(Sector(x=0.0, y=28.0, radius=38.0, start_angle=math.pi * 0.25, sweep_angle=math.pi * 0.55, fill_color="#E06C75", stroke_color="#7A3340", stroke_width=2.0))
    scene.add_node(face)
