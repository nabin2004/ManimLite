...
- **PyAV export / render:** Optional `frames_dir` writes numbered PNG frames during encode; `PyAVEncoder(linear_timeline=…)` drives linear easing; `SkiaRenderer.render_frame(..., ease=…)` (default `smoothstep`). CLI: `motiongram render --frames-dir`.
- **Procedural presets (optional):** `motiongram.procedural` — `RainyLandscapeManifest`, `materialize_rainy_landscape`, `apply_rainy_landscape_animations` (seedable world content + timeline wiring; not re-exported from `motiongram`). Refactored `examples/principles/26_world_viewport.py` to consume the manifest; tests in `tests/test_procedural_rainy_landscape.py`.
- **World coordinates:** `motiongram.world` — `WorldSpec`, `WorldPortal`, `world_to_screen` / `screen_to_world`, `world_pixel_affine_coeffs`, light-weight shell helpers (`world_shell`, `ground_strip`, `place_on_ground`, …) and fake-depth projection (`project_depth_fake`); `SemanticPart` / `WorldShellNodes` for simple character-style rigs. Scene graph: `Node.world_z` for depth ordering alongside `x` / `y`.
...
- **Shapes:** `Sector` and `SemiCircle` in `motiongram.shapes` (Skia-backed wedge fill via `fill_sector`).
- **Timeline recipes:** `motiongram.recipes` — `add_blink`, `add_squash_stretch_drop` as thin wrappers over `Scene.add_animation`.
...
- **CLI:** `motiongram render <scene.py> [-o OUT] [--width …] [--height …] [--fps …] [-q]` loads `build_scene()` or module `scene`, optional **`get_skia_renderer()`** hook for custom `SkiaRenderer(clear_color=…)`.
- **Animator:** `MoveY(y0, y1)` (symmetric with `MoveX`); exported from `motiongram`.
- **Public exports:** `PyAVEncoder`, `MoveY` in `motiongram.__all__`.
...
- **`lerp`**, **`smoothstep`**, **`MoveX`**, **`CircleOutline`**, **`Animator`** (protocol): exported from `motiongram`. `CircleOutline` applies only to `Circle` (otherwise `TypeError`).
...
- **`Circle`** (grid outline via polygon sampling in `core`); `motiongram.shapes.Circle` remains the Skia-oriented stub.
...
- **Branding:** Restored **MotionGram** as the project name and **`motiongram`** as the Python package / CLI (the **Typmotion** rename was reverted).
...
- **Export / CLI:** `PyAVEncoder.encode` and `motiongram render` are implemented (previously stubs).
...
- Repository scaffold: `src/motiongram` package stubs, tests layout, CI skeleton.
