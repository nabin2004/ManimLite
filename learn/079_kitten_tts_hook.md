...
Map the MotionGram repo’s **optional** `[tts]` story to the tutorial: local synthesis without cloud keys.
...
`VoiceOver` remains backend-agnostic. A `KittenVoiceOverBackend` (see repo `src/motiongram/audio/voiceover.py`) wraps the upstream wheel.
