...
Typmotion uses [Typst](https://typst.app/) instead of LaTeX for math typesetting.
...
3. The cache hashes the source, checks `~/.cache/typmotion/typst/` for an
...
```python
from typmotion import Scene, MoveX
...
The Typst SVG cache lives at `~/.cache/typmotion/typst/` by default.
Override with:
