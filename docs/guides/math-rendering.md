...
MotionGram uses [Typst](https://typst.app/) instead of LaTeX for math typesetting.
...
3. The cache hashes the source, checks `~/.cache/motiongram/typst/` for an
...
```python
from motiongram import Scene, MoveX
from motiongram.text import MathExpr
...
The Typst SVG cache lives at `~/.cache/motiongram/typst/` by default.
Override with:

```bash
export MOTIONGRAM_CACHE_HOME=/path/to/cache
```
