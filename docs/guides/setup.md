...
This guide covers installing Typmotion's two core external dependencies:
...
Typmotion uses Typst as a CLI tool (invoked via subprocess) to compile math
...
Ensure `typst` is on your `PATH`. Typmotion's `typst_cache.py` uses
...
| Math expressions render as blank | Check `typst --version` works; check `~/.cache/typmotion/typst/` for cached SVGs |
| `TypeError: MakeLinear(): incompatible function arguments` | Upgrade Typmotion — linear gradients pass a **sequence** of two `skia.Point`s per current skia-python bindings; ensure `skia-python>=120` per `pyproject.toml`. |
...
`typmotion render` calls it when present so MP4 matches `python your_scene.py`.
