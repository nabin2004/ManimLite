...
This document specifies the functional and non-functional requirements for **MotionGram**, a lightweight Python animation engine for educational video, optimized for small install size, fast cold rendering, and LLM-generated scene code.

### 1.2 Scope

MotionGram shall provide:
...
MotionGram is a standalone library and CLI. It is positioned as an alternative to ManimCE for **2D educational** animations where install size and cold start matter. It does not embed a slide framework; output is primarily **MP4** (optional WAV sidecar if needed).
...
| FR-12 | The system shall expose a **CLI**: `motiongram render <scene.py>` producing a video file path. |
...
| NFR-3 | **LLM ergonomics:** public API shall prefer **dataclasses**, explicit **constructors**, and **≤20** top-level symbols re-exported from `motiongram` (subject to revision with ADR). |
...
- **Python API:** package `motiongram` under `src/`, typed (`py.typed`).
- **CLI:** Typer-based entry point `motiongram`.
